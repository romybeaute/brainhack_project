...
def preparation(filename, state):
    path_to_file = f"{HOMEDIR}/src_data/HBN_dataset/%s/Video3_data.csv" % filename
    path_to_events = f"{HOMEDIR}/src_data/HBN_dataset/%s/Video3_event.csv" % filename
    # path_to_montage_glob = '/users/local/Venkatesh/HBN/GSN_HydroCel_129_hbn.sfp'
    path_to_montage_ses = (
        f"{HOMEDIR}/src_data/HBN_dataset/%s/Video3_chanlocs.csv" % filename
    )
    fs = 500
...
def preparation_resting_state(filename, state):
    path_to_file = f"{HOMEDIR}/src_data/HBN_dataset/%s/RestingState_data.csv" % filename
    path_to_events = (
        f"{HOMEDIR}/src_data/HBN_dataset/%s/RestingState_event.csv" % filename
    )
    # path_to_montage_glob = '/users/local/Venkatesh/HBN/GSN_HydroCel_129_hbn.sfp'
    path_to_montage_ses = (
        f"{HOMEDIR}/src_data/HBN_dataset/%s/RestingState_chanlocs.csv" % filename
    )
    fs = 500
    # chans_glob = mne.channels.read_custom_montage(
    #     fname='/users/local/Venkatesh/HBN/GSN_HydroCel_129_hbn.sfp')  # read_montage is deprecated
...
for i in range(1, len(subject_list) + 1):
    if not os.path.exists(
        f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/resting_state/{subject_list[i-1]}"
    ):
        os.makedirs(
            f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/resting_state/{subject_list[i-1]}"
        )
    ...
    resting_state_raw.save(
        f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/resting_state/{subject_list[i-1]}/raw.fif",
        overwrite=True,
    )
    np.savez_compressed(
        f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/resting_state/{subject_list[i-1]}/events.npz",
...
for i in range(1, len(subject_list) + 1):
    if not os.path.exists(
        f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/video-watching/{subject_list[i-1]}"
    ):
        os.makedirs(
            f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/video-watching/{subject_list[i-1]}"
        )
    ...
    sub_raw.save(
        f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/video-watching/{subject_list[i-1]}/raw.fif",
        overwrite=True,
    )
    np.savez_compressed(
        f"{HOMEDIR}/Generated_data/Pre_processed_HBN_dataset/video-watching/{subject_list[i-1]}/events.npz",
